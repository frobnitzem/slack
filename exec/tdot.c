...
void tensdot(const double beta,
             ...
             const double alpha,
              const double * restrict A, const int na, const int *sa, const uint8_t *pa,
              const double * restrict B, const int nb, const int *sb, const uint8_t *pb) {
    ...
    int outer, inner;
    const int n = (na+nb-nc)/2;   // number of contraction indices (nc =na + nb - 2*n)
    const int ntot = na + nb - n; // number of "expanded output" indices
                        // run-time index
    ...
    */
    ...
    if(n > 0) { // rolled inner loop
      ...
      for(int i=ain[n-1]+1; i<na; i++) Aistep *= sa[i];
      for(int i=bin[n-1]+1; i<nb; i++) Bistep *= sb[i];
      ...
      for(int I=0; I<outer; I++) {
        ...
        for(int J=0; J<inner; J++) {
            ...
            int j = Aind[0];
            for(int i=1; i<na; i++) // Horner
                j = j*sa[i] + Aind[i];
            int k = Bind[0];
            for(int i=1; i<nb; i++) // Horner
                k = k*sb[i] + Bind[i];
        ...
        for(int i=0; i<na-n; i++) { // outer indices assoc. to A
            Aind[aout[i]] = (Aind[aout[i]] + !((I+1)%Aoutb[i]))%sa[aout[i]];
        }
        for(int i=0; i<nb-n; i++) { // outer indices assoc. to B
...
int szprod(int *x, int n) {
    int k = 1; // last stride is 1 (C-ordering)
